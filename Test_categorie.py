...
for inhib in Ki_CB2_bis:
    if float(inhib) <= 2:
        #Tres fort
        Ki_CB2_categorize.append(1)
    if float(inhib) > 2:
        #Faible
        Ki_CB2_categorize.append(0)
...
class Bioactivity_net(object):
    ...
    def predict(self, x):
        x_new = smiles_to_arrays(x, self.max_length)
        return (self.model.predict(x_new).flatten() > 0.5).astype('int')
...
class BioactivityLSTM(Bioactivity_net):
    ...
    def network_initializer(self, max_timesteps=None):
        ...
        ### output layer
        out_layer = Dense(1, kernel_regularizer=L1L2(l2=self.l2), activation='sigmoid')(enc_output)
...
if __name__ == '__main__':
    ...
    #rgr = BioactivityCNN(vocab_size=76, max_length=300)

    x = np.asarray(smiles_CB2_bis)
    y = np.asarray(Ki_CB2_categorize,dtype=np.int)
    ...
    with open('Class_binaire_predictions_CB2.tsv', 'w') as f:
        f.write("SMILES\tCatégorie_réelle\tCatégorie_prédite\n")
        lis = [smiles_CB2, Ki_CB2_categorize, predictions]
        for x in zip(*lis):
            f.write("{0}\t{1}\t{2}\n".format(*x))"""